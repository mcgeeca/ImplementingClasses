...
class Point(object):
    """ Just started the definition of the class"""
    ...
    def move_to(self, x, y):
        self.sum_of_distances = self.sum_of_distances + self.get_distance_from(Point(x,y))
        self.x = x
        self.y = y
        self.n += 1

    def move_by(self, dx, dy):
        self.sum_of_distances = self.sum_of_distances + self.get_distance_from(Point(self.x+dx, self.y+dy))
        self.x = self.x + dx
        self.y = self.y + dy
        self.n += 1
    ...
    def halfway_to(self, p2):
        x_average = (p2.x + self.x) / 2
        y_average = (p2.y + self.y) / 2
        new_point = Point(x_average, y_average)
        return new_point
